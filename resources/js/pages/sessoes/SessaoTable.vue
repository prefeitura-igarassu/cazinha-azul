<template>
    <Page title="Sessões">
        <div class="grid mt-2">
            <Field   col="col-12 sm:col-3"  id="agendado"   campo="Agendado"   v-model:valor="params.agendado_para" type="date"           @changed="pesquisar" />
...
export default {
    ...
    computed: {
        filters(){
            ...
            if( this.params.agendado_para )
            {
                p.agendado_para = {
                    gt: this.params.agendado_para + " 00:00:00",
                    lt: this.params.agendado_para + " 23:59:59",
                };
            }
    ...
    methods: {
        ...
        visualizar( event ){
            this.$router.push( `/sessoes/${event.data.id}` );
        },

        imprimir( selected ){
            selected.forEach( sessao => {
                service.sessoes.imprimir( sessao.id );
            } );
        },
